...
// It is shared by both the single-symbol analyze command and the dashboard.
func fetchSymbolData(ctx context.Context, symbol string, svc *server.MarketDataService, ibClient *ibkr.Client) (*analysisv1.SingleStockData, error) {
	...
	// 2. Historical bars (~1 year daily). Pass empty endDate to avoid IB date-format errors.
...
func modelChainToProto(chain *model.OptionChain) []*marketdatav1.OptionChainExpiry {
	...
	for _, exp := range chain.Expirations {
		// Parse IB expiry format "YYYYMMDD" → compute DaysToExpiry
		dte := 0
