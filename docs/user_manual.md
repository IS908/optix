...
4. [技术指标计算方法](#4-技术指标计算方法)
5. [波动率体系](#5-波动率体系)
6. [趋势评分算法](#6-趋势评分算法)
7. [期权分析指标](#7-期权分析指标)
8. [策略推荐决策逻辑](#8-策略推荐决策逻辑)
9. [Black-Scholes 定价与希腊值](#9-black-scholes-定价与希腊值)
10. [评分体系](#10-评分体系)
11. [关键阈值速查表](#11-关键阈值速查表)
12. [实战使用流程](#12-实战使用流程)
13. [已知局限性与注意事项](#13-已知局限性与注意事项)
...
┌─────────────────────────────────────────────────┐
│                  CLI (Go)                        │
│  optix quote / analyze / dashboard / watch       │
└────────────────────┬────────────────────────────┘
                     │ gRPC
         ┌───────────┴───────────┐
         │                       │
┌────────▼────────┐    ┌─────────▼──────────────┐
│  IB TWS/Gateway │    │  Python Analysis Engine │
│  (市场数据源)    │    │  localhost:50052        │
│  port 7497      │    │  BS定价 / 技术分析 /    │
└─────────────────┘    │  期权分析 / 策略推荐    │
                       └─────────┬──────────────┘
                                 │
...
```bash
# 编译
go build -o optix ./cmd/optix-cli/
...
*本手册基于 Optix Phase 5 版本（2026-03-15）编写。系统用于辅助分析，不构成投资建议。期权交易存在亏损全部保证金的风险。*
